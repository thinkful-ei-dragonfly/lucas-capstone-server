...
stylesRouter
	...
	.post(bodyParser, (req, res, next) => {
		...
		StylesService.insertStyle(req.app.get('db'), newStyle)
			.then((style) => {
				res.status(201).json({
					id: style.id,
					post: xss(style.post),
...
stylesRouter
	.route('/:style_id')
	.get((req, res, next) => {
		StylesService.getById(req.app.get('db'), req.params.style_id)
			.then((style) => {
				if (!style) {
	...
	.delete((req, res, next) => {
		StylesService.deleteStyle(req.app.get('db'), req.params.style_id)
			.then(() => {
				res.status(204).end();
			})
			.catch(next);
